...
public class JoinPanel extends JPanel implements ActionListener,MouseListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			// Like => Regexp_like => index가 적용이 안 될 수 있다
			// 1. %가 앞에 있으면 안 됨
			// 2. 제어 => 함수 
			// 입력값 받기
			...
			{
				// 입력이 안 된 상태 => 유효성 검사 : 자바스크립트
				post.tf.requestFocus();
				return;
			}
			// 입력이 된 경우 => MemberDAO연결해서 데이터값 가지고 오기
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==post.table)
		{
			if(e.getClickCount()==2)
			{
				...
				// 값 첨부
				String zip=post.model.getValueAt(row, row).toString();
